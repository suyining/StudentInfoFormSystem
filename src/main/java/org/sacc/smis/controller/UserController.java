...
public class UserController {
    ...
    /**
     * 发送忘记密码邮件请求，每日申请次数不超过5次，每次申请间隔不低于30分钟
     * @param email
     * @param request
     * @return
     */
    ...
    public RestResult<String> sendValidationEmail(@ApiParam("邮箱地址") @RequestParam("email") String email,
        ...
        if (user == null){
            ...
            if (validateService.sendValidateLimitation(email,5,2)){//一天修改密码上限5次,每次30分钟.
                //插入一行数据,带有Token
                ...
                passWordResetEmail.setText("您正在申请重置密码，请点击此链接重置密码: \n" +
                        appUrl + "/validate/resetPassword?token=" + userValidate.getResetToken());
                validateService.sendPasswordResetEmail(passWordResetEmail);
                responseBody = RestResult.success(400,"发送完成");
            }else{
                responseBody = RestResult.error(500,"操作过于频繁,请稍后再试");
            }
        }
        return responseBody;
    }
